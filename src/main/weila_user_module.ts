...
export default class WLUserModule {
    constructor(private coreInterface: WL_CoreInterface) {
        this.coreInterface.registerPbMsgHandler(WL.Service.ServiceID.SERVICE_USER, this.onPbMsgHandler.bind(this));
    }
        ...
        if (serverMessage.serviceHead.commandType == WL.Service.CommandType.COMMAND_RESPONSE) {
            switch (serverMessage.serviceHead.commandId) {
                case WL.User.UserCommandId.USER_COMMAND_GET_USERINFO: {
                    ...
                    this.coreInterface.rspPbMsg(serverMessage.serviceHead.seq, serverMessage.serviceHead.resultCode, serverMessage.userMessage.rspGetUserInfo);
                }
                break;

                case WL.User.UserCommandId.USER_COMMAND_SET_CONFIG: {
                    this.coreInterface.rspPbMsg(serverMessage.serviceHead.seq, serverMessage.serviceHead.resultCode, serverMessage.userMessage.rspSetConfig);
                }
                break;
            }
    ...
    public async searchUserByNumber(key: string): Promise<WL_IDbUserInfo[]> {
        ...
        if (buildRet.resultCode == 0) {
            const rspGetUserInfo = await this.coreInterface.sendPbMsg(buildRet) as WL.User.IRspGetUserInfo;
            const dbUserList = [];
