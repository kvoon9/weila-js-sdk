...
import {WL_AnswerStatus, WL_ExtEventID, WL_FriendAnswerInviteInfo, WL_FriendInviteInfo} from "main/weila_external_data";
...
export default class WLFriendModule {
        ...
        if (serviceHead.commandType == WL.Service.CommandType.COMMAND_RESPONSE) {
            switch (serviceHead.commandId) {
                case WL.Friend.FriendCommandId.FRIEND_COMMAND_GET_FRIENDINFO: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.friendMessage ? serverMessage.friendMessage.rspGetFriendInfo : null);
                }
                break;
                ...
                case WL.Friend.FriendCommandId.FRIEND_COMMAND_GET_ONLINE_FRIEND: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.friendMessage ? serverMessage.friendMessage.rspGetOnlineFriend : null);
                }
                break;
            }
            ...
            if (serviceHead.commandId == WL.Friend.FriendCommandId.FRIEND_COMMAND_DELETE_FRIEND) {
                ...
                // 数据库删除好友后，发送通知告知客户端
                WeilaDB.getInstance().delFriendInfos([ntfDeleteFriend.userId]).then(value => {
                    this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_FRIEND_DELETED_IND, ntfDeleteFriend.userId);
                }).catch(reason => {
                    wlerr('删除好友:%d失败', ntfDeleteFriend.userId, reason);
                })
            }else if (serviceHead.commandId == WL.Friend.FriendCommandId.FRIEND_COMMAND_CHANGE_FRIENDINFO) {
                const rspFriendInfo = serverMessage.friendMessage.ntfChangeFriendInfo.friendInfo as WL.Friend.IFriendInfo;
                WeilaDB.getInstance().getFriendInfo(
                    serverMessage.friendMessage.ntfChangeFriendInfo.friendInfo.userId).then(async value => {
                ...
                (async () => {
                    const inviteInfo = serverMessage.friendMessage.ntfFriendInvite.inviteInfo as WL.Friend.FriendInviteInfo;
                    const userInfos = await WeilaDB.getInstance().convertFromUserRaw([serverMessage.friendMessage.ntfFriendInvite.userInfo]);
                    const inviteNotifyData = {} as WL_FriendInviteInfo;
                    await WeilaDB.getInstance().putUserInfo(userInfos[0]);
                    inviteNotifyData.inviterUserInfo = await WeilaDB.getInstance().getUser(userInfos[0].userId);
            ...
            }else if (serviceHead.commandId == WL.Friend.FriendCommandId.FRIEND_COMMAND_ANSWER_FRIEND_INVITE) {
                (async () => {
                    ...
                    friendAnswerInfo.inviteeUserInfo = await WeilaDB.getInstance().getUser(ntfFriendAnswerInvite.inviteeId);
    ...
    public async initFriends(): Promise<boolean> {
        ...
        let result = false;
        let friendSetting: WL_IDbSetting|undefined = await dbInstance.getSettingItem(WL_IDbSettingID.SETTING_FRIEND_LATEST_UPDATE);
        ...
        if (buildResult.resultCode == 0) {
            const rspGetFriendInfo = await this.coreInterface.sendPbMsg(buildResult, 30000) as WL.Friend.IRspGetFriendInfo;

            const deleteFriendIdList = rspGetFriendInfo.friendInfos.filter(value => {
    ...
    public async answerFriendInvite(inviterUserId: number, answerStatus: number, info: string): Promise<boolean> {
        const buildMsgRet = WeilaPbFriendWrapper.buildAnswerFriendInviteReq(inviterUserId, answerStatus, info);
    ...
    public async getOnlineFriends(): Promise<WL_IDbFriend[]> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rspGetOnlineFriend = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Friend.RspGetOnlineFriend;
    ...
    public async inviteFriendsForDevice(inviteeIds: number[], subUserId: number, detail: string): Promise<boolean> {
        for(let inviteeId of inviteeIds) {
            const buildMsgRet = WeilaPbFriendWrapper.buildFriendInviteReq(inviteeId, detail, undefined, subUserId);
            if (buildMsgRet.resultCode == 0) {
                await this.coreInterface.sendPbMsg(buildMsgRet);
            }
        }
    ...
    public async getDeviceSubFriends(subUserId: number): Promise<WL_IDbFriend[]> {
        ...
        if (buildMsgRet.resultCode == 0) {
            ...
            if (rsp.friendInfos) {
                const friendInfos: WL_IDbFriendInfo[] = await WeilaDB.getInstance().convertFromFriendRaw(rsp.friendInfos);
                const userInfos: WL_IDbUserInfo[] = await WeilaDB.getInstance().convertFromUserRaw(rsp.userInfos);
