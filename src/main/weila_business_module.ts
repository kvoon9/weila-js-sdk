...
export default class WLBusinessModule {
		...
		if (serviceHead.commandType == WL.Service.CommandType.COMMAND_RESPONSE) {
			switch (serviceHead.commandId) {
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_SEARCH_STAFFS: {
					this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode,
						serviceHead.resultCode == 0 ? serverMessage.businessMessage.rspStaffSearchStaffs : null);
				}
				break;
			...
			switch (serviceHead.commandId) {
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_COMMON_SESSION_CHANGE: {
					...
					const serviceSessionInfo = WeilaDB.getInstance().convertFromServiceSessionRaw(ntf.session);
					...
					if (serviceSessionInfo.serviceSession.serviceStatus == WL_ServiceStatus.SERVICE_STATUS_INIT) {
						(async () => {
							...
							await WeilaDB.getInstance().putServiceStaffs(WeilaDB.getInstance().convertFromStaffRaws(ntf.session.staffAttributes));
							await WeilaDB.getInstance().putUserInfos(serviceSessionInfo.staffs.map(value => {
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_COMMON_STAFF_INVITE: {
					...
					staffInvite.invitorInfo.staff = WeilaDB.getInstance().convertFromStaffRaws([ntf.inviteInfo.invitor])[0];
					staffInvite.invitorInfo.userData = WeilaDB.getInstance().convertFromUserRaw([ntf.inviteInfo.invitor.userInfo])[0];
					staffInvite.content = ntf.inviteInfo.content;
					staffInvite.service = WeilaDB.getInstance().convertFromServiceRaw(ntf.inviteInfo.serviceAttribute);

					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_COMMON_STAFF_INVITE_IND, staffInvite);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_COMMON_ANSWER_STAFF_INVITE: {
					...
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_COMMON_ANSWER_INVITE_IND, commonAnswerStaffInvite);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_ACCEPT_SESSION: {
					...
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_ACCEPT_SESSION_IND, staffSessionInfo);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_EXIT_SESSION: {
					...
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_EXIT_SESSION_IND, staffSessionInfo);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_CLOSE_SESSION: {
					...
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_CLOSE_SESSION_IND, staffSessionInfo);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_SESSION_INVITE: {
					...
					staffSessionInvite.session = WeilaDB.getInstance().convertFromServiceSessionRaw(ntf.session);
					staffSessionInvite.content = ntf.content;
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_SESSION_INVITE_IND, staffSessionInvite);
				}
				break;
				...
				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_ANSWER_SESSION_INVITE: {
					...
					staffAnswerSessionInvite.invitee.staff = WeilaDB.getInstance().convertFromStaffRaws([ntf.invitee])[0];
					staffAnswerSessionInvite.invitee.userData = WeilaDB.getInstance().convertFromUserRaw([ntf.invitee.userInfo])[0];

					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_ANSWER_INVITE_IND, staffAnswerSessionInvite);
				}
				break;

				case WL.Business.BusinessCommandId.BUSINESS_COMMAND_STAFF_REMOVE_SESSION_STAFFS: {
					const ntf = serverMessage.businessMessage.ntfStaffRemoveSessionStaff;
					this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_STAFF_REMOVED_SESSION_IND, Long.fromValue(ntf.sessionId).toString(10));
				}
				break;
			}
		}
	...
	public async getSessionServiceFromServer(sessionId: string): Promise<WL_IDbServiceSessionInfo|undefined> {
		...
		if (buildMsgRet.resultCode == 0) {
			...
			if (rsp.sessions.length) {
				const serviceSessionInfo = WeilaDB.getInstance().convertFromServiceSessionRaw(rsp.sessions[0]);
				...
				await WeilaDB.getInstance().putServiceStaffs(WeilaDB.getInstance().convertFromStaffRaws(rsp.sessions[0].staffAttributes));
				await WeilaDB.getInstance().putService(serviceSessionInfo.service);
		...
		if (buildMsgRet.resultCode == 0) {
			...
			for (let dbSession of dbSessionList) {
				...
				if (idx !== -1) {
					...
					const serviceSessionInfo = WeilaDB.getInstance().convertFromServiceSessionRaw(serviceSessionRaw);
	...
	public async commonStaffInvite(serviceId: number, userIds: number[], content?: string): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildCommonStaffInviteReq(serviceId, userIds, content);
		if (buildMsgRet.resultCode == 0) {
	...
	public async commonAnswerStaffInvite(serviceId: number, invitorId: number, status: WL_AnswerStatus): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildCommonAnswerStaffInviteReq(serviceId, invitorId, status);
		if (buildMsgRet.resultCode == 0) {
			return this.coreInterface.sendPbMsg(buildMsgRet);
		}
		return Promise.reject(new Error('创建消息出错'));
	...
	public async commonGetStaffInvite(latestUpdate: number): Promise<WL_StaffInvite[]> {
		...
		if (buildMsgRet.resultCode == 0) {
			...
			const rsp = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Business.IRspCommonGetStaffInvite;
			if (rsp.invites) {
				for(let invite of rsp.invites) {
					const staffInvite = {} as WL_StaffInvite;
					staffInvite.service = WeilaDB.getInstance().convertFromServiceRaw(invite.serviceAttribute);
					staffInvite.invitorInfo = {} as WL_IDbServiceStaffInfo;
					staffInvite.invitorInfo.staff = WeilaDB.getInstance().convertFromStaffRaws([invite.invitor])[0];
					staffInvite.invitorInfo.userData = WeilaDB.getInstance().convertFromUserRaw([invite.invitor.userInfo])[0];
	...
	public async staffAcceptSession(sessionId: string): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildStaffAcceptSessionReq(Long.fromValue(sessionId));
		if (buildMsgRet.resultCode == 0) {
			return this.coreInterface.sendPbMsg(buildMsgRet);
		}
	...
	public async staffSessionInvite(sessionId: string, userIds: number[], content?: string): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildStaffSessionInviteReq(Long.fromValue(sessionId), userIds, content);
		if (buildMsgRet.resultCode == 0) {
			return this.coreInterface.sendPbMsg(buildMsgRet);
		}
	...
	public async staffAnswerSessionInvite(sessionId: string, status: WL_AnswerStatus): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildStaffAnswerSessionInviteReq(Long.fromValue(sessionId), status);
		if (buildMsgRet.resultCode == 0) {
			return this.coreInterface.sendPbMsg(buildMsgRet);
		}
	...
	public async staffRemoveSessionStaff(sessionId: string, staffIds: number[]): Promise<boolean> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildStaffRemoveSessionStaffReq(Long.fromValue(sessionId), staffIds);
		if (buildMsgRet.resultCode == 0) {
			return this.coreInterface.sendPbMsg(buildMsgRet);
		}
	...
	public async staffSearchStaffs(serviceId: number, pageIndex: number, pageSize: number, staffClass?: number): Promise<WL_IDbServiceStaffInfo[]> {
		const buildMsgRet = WeilaPbBusinessWrapper.buildStaffSearchStaffsReq(serviceId, pageIndex, pageSize, staffClass);
		if (buildMsgRet.resultCode == 0) {
			const rsp: WL.Business.IRspStaffSearchStaffs = await this.coreInterface.sendPbMsg(buildMsgRet);
			if (rsp.staffs) {
				const staffs = WeilaDB.getInstance().convertFromStaffRaws(rsp.staffs);
				const userInfos = WeilaDB.getInstance().convertFromUserRaw(rsp.staffs.map(value => {
