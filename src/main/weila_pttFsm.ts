...
import {WL_IDbAudioData, WL_IDbMsgData, WL_IDbMsgDataStatus, WL_IDbMsgDataType} from "db/weila_db_data";
...
import {WL_PttAudioPlaySource, WL_PttAudioPlayState, WL_PttPlayIndData} from "main/weila_external_data";
...
export default class WLPttFsm {
    ...
    constructor(private pttFsmListener: WL_PttFsmListener) {
        ...
        this.pttFsm = pttFsm.withContext({
        ...
        this.pttFsmService.onTransition((state, event) => {
            wllog('事件:%s 从状态:%s ===> 到状态:%s', event.type, state.history ? JSON.stringify(state.history.value) : '空', JSON.stringify(state.value));
        }).onEvent(event => {
    ...
    onPttPayloadPlaying(context: any, event: any, actionMeta: any) {
        ...
        if (event.type == "FSM_FEED_PTT_PAYLOAD_EVT") {
            ...
            WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_START_PLAY_ID).then(playRet => {
                wllog('播放铃声成功');
            }).finally(() => {
                this.onPlayInd(audioItem, audioItem.playerType, WL_PttAudioPlayState.PTT_AUDIO_PLAYING_START);
                setTimeout(() => {
                    this.feedAudioPayload(audioItem, audioItem.playerType);
                }, this.PLAY_DELAY)
    ...
    onPlayItemFinish(context: any, event: any, actionMeta: any) {
        ...
        WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_STOP_PLAY_ID).finally(() => {
            ...
            this.onPlayInd(null, WLPlayerType.WL_PTT_STREAM_PLAYER, WL_PttAudioPlayState.PTT_AUDIO_PLAYING_END);
        });
    }

    private onPlayInd(audioItem: WL_PttAudioItem|null, playerType: WLPlayerType, state: WL_PttAudioPlayState) {
    ...
    onSinglePlayEntry(context: any, event: any, actionMeta: any) {
        ...
        this.audioManager.openPlayer(WLPlayerType.WL_PTT_SINGLE_PLAYER, 16000).then(openRet => {
            WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_START_PLAY_ID).then(playRet => {
                ...
                this.audioManager.startPlay(WLPlayerType.WL_PTT_SINGLE_PLAYER).then(startRet => {
                    //TODO:
                    this.singlePttPlayItem = event.data as WL_PttAudioItem;
                    this.onPlayInd(this.singlePttPlayItem, WLPlayerType.WL_PTT_SINGLE_PLAYER, WL_PttAudioPlayState.PTT_AUDIO_PLAYING_START);
                    setTimeout(() => {
    ...
    onEnterRealtime(context: any, event: any, actionMeta: any) {
        ...
        if (event.type == 'FSM_PLAY_REALTIME_EVT') {
            ...
            this.audioManager.openPlayer(audioItem.playerType, 16000).then(openRet => {
                WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_START_PLAY_ID).then(playRet => {
                    wllog('播放铃声成功');
                }).finally(() => {
                    this.audioManager.startPlay(audioItem.playerType).then(startRet => {
                        wllog('播放器开始:', startRet);
                        this.streamPttPlayItem.msgData.status = WL_IDbMsgDataStatus.WL_DB_MSG_DATA_STATUS_READ;
                        if (this.streamPttPlayItem.shouldSave) {
                            WeilaDB.getInstance().putMsgData(this.streamPttPlayItem.msgData).then(value => {
                                wllog('更新消息结果', value);
                            })
                        }
    ...
    onHistoryPlaying(context: any, event: any, actionMeta: any) {
        if (event.type == fsmEvents.FSM_PLAY_HISTORY_EVT) {
            ...
            this.audioManager.openPlayer(WLPlayerType.WL_PTT_HISTORY_PLAYER, 16000).then(openRet => {
                WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_START_PLAY_ID).then(playRet => {
                    wllog('播放铃声成功');
                }).finally(() => {
                    if (!this.audioManager.isPlayerStarted(WLPlayerType.WL_PTT_HISTORY_PLAYER)) {
                        this.audioManager.startPlay(WLPlayerType.WL_PTT_HISTORY_PLAYER).then(startRet => {
                            ...
                            this.onPlayInd(this.historyPttPlayItems[this.historyPttPlayIndex],
                                WLPlayerType.WL_PTT_HISTORY_PLAYER, WL_PttAudioPlayState.PTT_AUDIO_PLAYING_START);
    ...
    onPlayHistoryItemFinish(context: any, event: any, actionMeta: any) {
        ...
        WeilaRingPlayer.weila_playRing(WL_ConfigID.WL_RES_RING_STOP_PLAY_ID).finally(() => {
            this.pttFsmService.send(fsmEvents.FSM_PTT_CHECK_RESOURCE_EVT);
            this.onPlayInd(event.data, WLPlayerType.WL_PTT_HISTORY_PLAYER, WL_PttAudioPlayState.PTT_AUDIO_PLAYING_END);
        });
    }
    ...
    feedAudioPayload(audioItem: WL_PttAudioItem, playerType: WLPlayerType): boolean {
        wllog('喂播放器:%d 数据:', playerType, audioItem.playIndex, audioItem.payloadList.length, audioItem.isCompleted);
    ...
    private onAudioMessage(event: WLAudioEvent, data: any): void {
        switch (event) {
            ...
            case WLAudioEvent.WL_AUDIO_OPUS_CODED_DATA_IND: {
                if (this.talkingInfo.curMarker == WL_PttPackType.PTT_END_PACK ||
                ...
                if (this.talkingInfo.frameBufferCache.length >= 25) {
                    this.packetPttMsg(this.talkingInfo.frameBufferCache.slice(0, WLPttFsm.FRAME_COUNT), false);
                ...
                if (audioItem) {
                    ...
                    if (result) {
                        ...
                        if (audioItem.shouldSave) {
                            WeilaDB.getInstance().putMsgData(audioItem.msgData).then(value => {
    ...
    async notifyPttDataReady(): Promise<void> {
        ...
        if (this.pttFsmService.state.matches('idle') || this.pttFsmService.state.matches('realtimePlaying.waitPayload')) {
    ...
    putAudioMsgData(msgData: WL_IDbMsgData, shouldSave: boolean) {
        if (msgData.msgType == WL_IDbMsgDataType.WL_DB_MSG_DATA_AUDIO_TYPE) {
            ...
            if (msgData.audioData.data) {
                ...
                fetchWithTimeout(newUrl, {method: 'GET', mode: 'cors'}, 5000).then(async result => {
                    if (result.ok) {
                        try {
    ...
    private packetLastPttMsg(): void {
        if (this.talkingInfo.frameBufferCache.length > 0) {
            while (this.talkingInfo.frameBufferCache.length >= WLPttFsm.FRAME_COUNT) {
                this.packetPttMsg(this.talkingInfo.frameBufferCache.slice(0, WLPttFsm.FRAME_COUNT),
