...
export default class WLLoginModule {
            ...
            switch (serverMessage.serviceHead!.commandId) {
                case WL.Login.LoginCommandId.LOGIN_COMMAND_LOGIN_APP: {
                    if (serviceHead.resultCode == 0) {
                        WeilaDB.init(serverMessage.loginMessage!.rspLoginApp!.userInfo!.userId!.toString(), 3);
                    }
                ...
                case WL.Login.LoginCommandId.LOGIN_COMMAND_BIND_EXTENSION: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.loginMessage ? serverMessage.loginMessage.rspBindExtension : null);
                }
                break;

                case WL.Login.LoginCommandId.LOGIN_COMMAND_UNBIND_EXTENSION: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.loginMessage ? serverMessage.loginMessage.rspUnbindExtension : null)
                }
                break;
            }
            ...
            switch (serverMessage.serviceHead.commandId) {
                case WL.Login.LoginCommandId.LOGIN_COMMAND_ANSWER_BIND_EXTENSION: {
                    (async () => {
                        ...
                        bindAnswerInfo.deviceInfo = WeilaDB.getInstance().convertFromExtensionInfoRaw(ntf.extensionInfo);
                        bindAnswerInfo.verifyCode = ntf.verificationCode;

                        this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_DEVICE_BINDING_ANSWER_IND, bindAnswerInfo);
                    })();
                }
                break;
            }
    ...
    public async loginReq(account: string, password: string, countryCode: string): Promise<WL_IDbUserInfo> {
        wllog('登录请求:', account, password, countryCode);
        const appKeyAndId = WeilaDB.getAppAuthInfo() ? WeilaDB.getAppAuthInfo() : getAppKeyAndId();
        const wlBuildMsgRet = WeilaPBLoginWrapper.buildLoginAppReq(account, password, countryCode, appKeyAndId.appId, appKeyAndId.appKey);
        if (wlBuildMsgRet.resultCode == 0) {
            const loginAppRsp = await this.coreInterface.sendPbMsg(wlBuildMsgRet, 10000) as WL.Login.IRspLoginApp;
    ...
    public async bindDevice(verifyCode: string): Promise<number> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rspBindMsg = await this.coreInterface.sendPbMsg(buildMsgRet, 30000) as WL.Login.RspBindExtension;
    ...
    public async unbindDevice(deviceUserId: number): Promise<number> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rspUnbindMsg = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Login.RspUnbindExtension;
    ...
    public async initExtension(): Promise<boolean> {
        let subUserVerItem: WL_IDbSetting|undefined = await WeilaDB.getInstance().getSettingItem(WL_IDbSettingID.SETTING_SUB_USERINFO_VER_UPDATE);
        ...
        if (buildMsgRet.resultCode == 0) {
            const rsp: WL.Login.IRspGetExtensionList = await this.coreInterface.sendPbMsg(buildMsgRet, 30000);
    ...
    public async setDeviceConfig(cfgKey: string, cfgValue: string, subUserId: number): Promise<string> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rspConfig = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Login.IRspSetExtensionConfig;
