...
class WLRecorderWorklet extends AudioWorkletProcessor {
        ...
        switch (ev.event) {
            case WLAudioRecorderEvent.WL_WORKLET_NODE_OPEN_REQ: {
                ...
                opus(Module).then(async value => {
                    ...
                    if (!this.encoder) {
                        this.reset();
                        this.reportRspEvent(WLAudioRecorderEvent.WL_WORKLET_NODE_OPEN_RSP, false, '编码器初始化失败');
                        return;
                    }

                    this.pcmBuffer = this.opusModule._malloc(this.processFrameCount * Int16Array.BYTES_PER_ELEMENT);
                    if (!this.pcmBuffer) {
                        this.reset();
                        this.reportRspEvent(WLAudioRecorderEvent.WL_WORKLET_NODE_OPEN_RSP, false, '申请内存失败');
                        return;
                    }
                    ...
                    this.reportRspEvent(WLAudioRecorderEvent.WL_WORKLET_NODE_OPEN_RSP, false, '出现异常:' + reason);
                    this.reset();
                });
            }
            break;
    ...
    encodeData() {
        if (this.state === RECORD_STARTED) {
            ...
            const encodedLen = this.opusModule._Opus_encode(this.encoder, this.pcmBuffer, this.processFrameCount, this.encodedData, 128);
            if (encodedLen > 0) {
                const dataBuffer = new Uint8Array(this.opusModule.HEAPU8.buffer, this.encodedData, encodedLen);
                ...
                if (this.transportWithList) {
                    ...
                    try {
                        this.port.postMessage({
                            event: WLAudioRecorderEvent.WL_WORKLET_NODE_DATA_IND,
                            data: {type:'buffer', data:dataBuffer}}, [dataBuffer.buffer]);
                    }catch (e) {
    ...
    process(inputs, outputs, params) {
        ...
        this.frameRingBuffer.push(new Int16Array(inputBuffer.map(v => {return Math.ceil(v * 32768);})));
