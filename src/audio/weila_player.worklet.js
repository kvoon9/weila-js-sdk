                ...
                if (this.opusModule == null) {
                    ...
                    opus(Module).then(value => {
                        if (this.state === PLAY_CLOSING) {
                            this.reset();
                            this.reportRsp(WLAudioPlayerEvent.WL_WORKLET_NODE_OPEN_RSP, false, '编码器初始化失败');
                            return;
                        }
                        ...
                        if (!this.pcmBuffer) {
                            this.reportRsp(WLAudioPlayerEvent.WL_WORKLET_NODE_OPEN_RSP, false, '申请缓存内存失败');
                            this.reset();
                            this.state = PLAY_CLOSED;
                            return;
                        }
                        ...
                        if (!this.encodedData) {
                            this.reportRsp(WLAudioPlayerEvent.WL_WORKLET_NODE_OPEN_RSP, false, '申请编码内存失败');
                            this.reset();
                            this.state = PLAY_CLOSED;
                            return;
                        }
        ...
        if (this.pcmRingBuffer && this.pcmRingBuffer.frameAvailable >= output.length) {
            ...
            if (this.noMoreData) {
                ...
                if (this.pcmRingBuffer.frameAvailable) {
                    ...
                    this.sendEvent(WLAudioPlayerEvent.WL_WORKLET_NODE_FINISH_PLAY, {playerType: this.playerType});
                    this.caching = true;
                    this.noMoreData = false;
                }
            }
