...
import {WLAudioEvent, WLAudioPlayerEvent, WLAudioRecorderEvent, WLPlayerType} from "./weila_audio_data";
...
class WLAudio {
    ...
    constructor() {
        ...
        this.playState = [WLPlayerState.WL_PLAYER_CLOSED, WLPlayerState.WL_PLAYER_CLOSED, WLPlayerState.WL_PLAYER_CLOSED];
    ...
    public async openPlayer(playerType: WLPlayerType, sampleRate: number): Promise<boolean> {
        ...
        try {
            if (this.playerWorkNode[playerType] == null) {
                ...
                this.playerWorkNode[playerType] = new AudioWorkletNode(this.audioContext, 'weila_player_processor', {
                    channelCount: 2,
                    processorOptions: {
        ...
        return new Promise<boolean>((resolve, reject) => {
            this.sendPlayerEvent(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_OPEN_REQ, {sampleRate: sampleRate});
            const callback = {} as WLAudioCallback;
            callback.resolve = resolve;
            callback.reject = reject;
            callback.waitRspTimer = setTimeout((cb) => {
                if (cb.reject) {
    ...
    private playOperation(playerType: WLPlayerType, reqEvent: WLAudioPlayerEvent,
                          rspEvent: WLAudioPlayerEvent, data: any): Promise<boolean> {
    ...
    private startAudioSource(playerType: WLPlayerType): void {
        if (this.playerSource[playerType] == null) {
            const audioBuffer = this.audioContext!.createBuffer(1, 16000, this.playSampleRate[playerType]);
    ...
    public startPlay(playerType: WLPlayerType): Promise<boolean> {
        ...
        return this.playOperation(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_START_REQ,
            WLAudioPlayerEvent.WL_WORKLET_NODE_START_RSP, null);
    }

    public pausePlay(playerType: WLPlayerType): Promise<boolean> {
        wllog('播放器:%d 暂停播放', playerType);
        if (this.playState[playerType] !== WLPlayerState.WL_PLAYER_STARTED &&
        this.playState[playerType] !== WLPlayerState.WL_PLAYER_STARTING) {
            return Promise.reject(new Error('播放器状态不正确:' + this.playState[playerType]));
        }

        this.playState[playerType] = WLPlayerState.WL_PLAYER_PAUSING;
        return this.playOperation(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_PAUSE_REQ,
            WLAudioPlayerEvent.WL_WORKLET_NODE_PAUSE_RSP, null);
    }

    public resumePlay(playerType: WLPlayerType): Promise<boolean> {
        wllog('播放器:%d 恢复播放', playerType);
        if (this.playState[playerType] !== WLPlayerState.WL_PLAYER_PAUSED &&
            this.playState[playerType] !== WLPlayerState.WL_PLAYER_PAUSING) {
            return Promise.reject(new Error('播放器状态不正确:' + this.playState[playerType]));
        }
        this.startAudioSource(playerType);
        this.playState[playerType] = WLPlayerState.WL_PLAYER_RESUMING;
        return this.playOperation(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_RESUME_REQ,
            WLAudioPlayerEvent.WL_WORKLET_NODE_RESUME_RSP, null);
    }

    public stopPlay(playerType: WLPlayerType): Promise<boolean> {
        wllog('播放器:%d 停止播放', playerType);
        if (this.playState[playerType] !== WLPlayerState.WL_PLAYER_STARTING &&
        ...
        return this.playOperation(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_STOP_REQ,
            WLAudioPlayerEvent.WL_WORKLET_NODE_STOP_RSP, null);
    }
    ...
    public closePlayer(playerType: WLPlayerType): Promise<boolean> {
        ...
        this.playState[playerType] = WLPlayerState.WL_PLAYER_CLOSING;
        return this.playOperation(playerType, WLAudioPlayerEvent.WL_WORKLET_NODE_CLOSE_REQ,
            WLAudioPlayerEvent.WL_WORKLET_NODE_CLOSE_RSP, null);
    }
    ...
    public async openRecorder(sampleRate: number, bitRate: number): Promise<boolean> {
        ...
        try {
            ...
            if (this.recorderWorkNode == null) {
                const configData = await WLConfig.getConfigData(WL_ConfigID.WL_RES_DATA_OPUS_WASM_ID);
                // 初始化录音器线程
                this.recorderWorkNode = new AudioWorkletNode(this.audioContext, "weila_recorder_processor", {
        ...
        return new Promise<boolean>((resolve, reject) => {
            ...
            navigator.getUserMedia({audio: true}, stream => {
                ...
                this.sendRecorderEvent(WLAudioRecorderEvent.WL_WORKLET_NODE_OPEN_REQ, {sampleRate: sampleRate, bitRate: bitRate});
                const callback = {} as WLAudioCallback;
                callback.resolve = resolve;
                callback.reject = reject;
                callback.waitRspTimer = setTimeout((cb) => {
    ...
    private recordOperation(reqEvent: WLAudioRecorderEvent, rspEvent: WLAudioRecorderEvent, data: any): Promise<boolean> {
    ...
    public startRecord(): Promise<boolean> {
        ...
        return this.recordOperation(WLAudioRecorderEvent.WL_WORKLET_NODE_START_REQ,
            WLAudioRecorderEvent.WL_WORKLET_NODE_START_RSP, null);
    }

    public stopRecord(): Promise<boolean> {
        wllog('停止录音');
        if (this.recordState !== WLRecorderState.WL_RECORDER_STARTING &&
        this.recordState !== WLRecorderState.WL_RECORDER_STARTED) {
            return Promise.reject(new Error('录音器状态不正确:' + this.playState));
        }

        this.recordState = WLRecorderState.WL_RECORDER_STOPPING;
        return this.recordOperation(WLAudioRecorderEvent.WL_WORKLET_NODE_STOP_REQ,
            WLAudioRecorderEvent.WL_WORKLET_NODE_STOP_RSP, null);
    }

    public closeRecorder(): Promise<boolean> {
        wllog('关闭录音器');
        if (this.recordState === WLRecorderState.WL_RECORDER_CLOSING ||
            this.recordState === WLRecorderState.WL_RECORDER_CLOSED) {
            return Promise.reject(new Error('录音器状态不正确:' + this.playState));
        }

        this.recordState = WLRecorderState.WL_RECORDER_CLOSING;
        return this.recordOperation(WLAudioRecorderEvent.WL_WORKLET_NODE_CLOSE_REQ,
            WLAudioRecorderEvent.WL_WORKLET_NODE_CLOSE_RSP, null);
    }
}
