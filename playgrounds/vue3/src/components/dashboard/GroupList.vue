...
<template>
  <div class="flex flex-col h-full">
    <!-- Header -->
    <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between bg-white shrink-0">
      <h2 class="text-base font-semibold text-gray-900">群组</h2>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <!-- Empty State -->
      <div v-else-if="!groups || groups.length === 0" class="flex flex-col items-center justify-center h-full text-gray-400">
      ...
      <div v-else class="p-2 space-y-1">
        <button v-for="group in groups" :key="group.groupId"
          ...
          <div class="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden shrink-0">
            ...
            <span v-else class="icon-[carbon--group] text-xl text-gray-400"></span>
          </div>
          <!-- Info -->
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-500 mt-0.5">{{ group.memberCount }} 位成员</p>
          </div>
          <!-- Arrow -->
          <span class="icon-[carbon--chevron-right] text-gray-400 shrink-0"></span>
        </button>
      </div>
    </div>
  </div>
