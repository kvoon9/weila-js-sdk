...
import {
    ...
    WL_GroupInviteInfo,
    WL_GroupJoinInfo, WL_GroupMemberDeleteInfo
...
export default class WLGroupModule {
        ...
        if (serviceHead.commandType == WL.Service.CommandType.COMMAND_RESPONSE) {
            switch (serviceHead.commandId) {
                case WL.Group.GroupCommandId.GROUP_COMMAND_GET_GROUPVERSION: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspGetGroupVersion);
                }
                break;

                case WL.Group.GroupCommandId.GROUP_COMMAND_GET_GROUPATTRIBUTE: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspGetGroupAttribute);
                }
                break;
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_BROADCAST: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspGroupBroadcast);
                }
                break;
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_GET_ONLINE_MEMBER: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspGetGroupOnlineMember);
                }
                break;
                case WL.Group.GroupCommandId.GROUP_COMMAND_CREATE_GROUP: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspCreateGroup);
                }
                break;
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_CHANGE_GROUPATTRIBUTE: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspChangeGroupAttribute);
                }
                break;

                case WL.Group.GroupCommandId.GROUP_COMMAND_GET_SUB_GROUPATTRIBUTE: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage.rspGetSubGroupAttribute);
                }
                break;

                case WL.Group.GroupCommandId.GROUP_COMMAND_GROUP_INVITE: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage ? serverMessage.groupMessage.rspGroupInvite : null);
                }
                break;

                case WL.Group.GroupCommandId.GROUP_COMMAND_ADD_SUB_MEMBER: {
                    this.coreInterface.rspPbMsg(serviceHead.seq, serviceHead.resultCode, serverMessage.groupMessage ? serverMessage.groupMessage.rspAddSubMember : null);
                }
                break;
            }
            ...
            switch (serviceHead.commandId) {
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_GROUP_JOIN: {
                    ...
                    (async () => {
                        ...
                        groupJoinInfo.groupInfo = await WeilaDB.getInstance().getGroup(ntfGroupJoin.groupId.toString(10));
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_CHANGE_GROUPATTRIBUTE: {
                    ...
                    WeilaDB.getInstance().getGroup(groupId).then(async value => {
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_CHANGE_MEMBERINFO: {
                    const ntfChangeMemberInfo = serverMessage.groupMessage.ntfChangeMemberInfo as WL.Group.INtfChangeMemberInfo;
                    const groupId = ntfChangeMemberInfo.groupId.toString(10);
                    ntfChangeMemberInfo.memberInfo

                    WeilaDB.getInstance().getGroupMemberInfo(groupId, ntfChangeMemberInfo.memberInfo.userId).then(async value => {
                        ...
                        if (memberInfo) {
                            ...
                            if (ntfChangeMemberInfo.memberInfo.shutupTimeout !== undefined) {
                                memberInfo.speechDisableTimeout = Long.fromValue(ntfChangeMemberInfo.memberInfo.shutupTimeout).toNumber();
                            }
                            ...
                            wllog('群成员变更了');
                            this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_GROUP_MEMBERS_MODIFIED_IND, memberInfo);
                        }
                    }).catch(reason => {
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_DELETE_GROUP: {
                    ...
                    //删除群并且通知
                    WeilaDB.getInstance().delGroups([groupId]).then(() => {
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_DELETE_MEMBER: {
                    ...
                    WeilaDB.getInstance().delMemberInfos(ntfDeleteMember.groupId.toString(10),
                        ntfDeleteMember.userIdList).then(value => {
                        wllog('删除了群成员成功');
                        // 通知外部
                        ...
                        this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_GROUP_MEMBER_DELETED_IND, info);
                    }).catch(reason => {
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_NEW_MEMBER: {
                    ...
                    (async () => {
                        ...
                        if (groupInfo) {
                            const memberInfos = WeilaDB.getInstance().convertFromMemberRaw(groupId, ntfNewMember.memberInfoList);
                            const userInfos = WeilaDB.getInstance().convertFromUserRaw(ntfNewMember.userInfoList);
                        ...
                        this.coreInterface.sendExtEvent(WL_ExtEventID.WL_EXT_GROUP_NEW_MEMBER_JOINED_IND, ntfNewMember.memberInfoList);
                    })().catch(reason => {
                        wlerr('保存信息出错!', reason);
                    });
                }
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_ANSWER_GROUP_JOIN: {
                    ...
                    (async () => {
                        ...
                        answerGroupJoin.groupInfo = await WeilaDB.getInstance().getGroup(ntfAnswerJoin.groupId.toString(10));
                        answerGroupJoin.joinUserInfo = await WeilaDB.getInstance().getUser(ntfAnswerJoin.joinId);
                        answerGroupJoin.answerUserInfo = await WeilaDB.getInstance().getUser(ntfAnswerJoin.answerId);
                ...
                case WL.Group.GroupCommandId.GROUP_COMMAND_ANSWER_GROUP_INVITE: {
                    ...
                    (async () => {
                        ...
                        answerGroupInvite.groupInfo = await WeilaDB.getInstance().getGroup(ntfAnswerInvite.groupId.toString(10));
                        answerGroupInvite.invitorUserInfo = await WeilaDB.getInstance().getUser(ntfAnswerInvite.inviterId);
                        answerGroupInvite.inviteeUserInfo = await WeilaDB.getInstance().getUser(ntfAnswerInvite.inviteeId);
                        answerGroupInvite.status = ntfAnswerInvite.status;
                        answerGroupInvite.updated = Long.fromValue(ntfAnswerInvite.updated).toNumber();

                        const notification: WL_IDbNotification = {} as WL_IDbNotification;
                        notification.notificationType = WL_IDbNotificationType.GROUP_ANSWER_INVITE_NOTIFICATION;
    ...
    public async initGroups(): Promise<boolean> {
        ...
        if (getGroupVersionBuild.resultCode == 0) {
            ...
            groupVerMsgData = await this.coreInterface.sendPbMsg(getGroupVersionBuild, 30000) as WL.Group.IRspGetGroupVersion;
            ...
            const invalidGroups = oldGroups.filter(value => {
            ...
            for(let i = 0; i < groupVerMsgData.versionInfoList.length; i++) {
                ...
                wllog('获取群信息:', dbGroup);
                if (!dbGroup || dbGroup.memberVersion != ver.memberVersion || dbGroup.version != ver.groupVersion) {
                    const buildGroupInfo = WeilaPBGroupWrapper.buildGetGroupInfoReq(Long.fromValue(ver.groupId), 0);
                    if (buildGroupInfo.resultCode == 0) {
                        const rspGroupInfo = await this.coreInterface.sendPbMsg(buildGroupInfo, 30000) as WL.Group.IRspGetGroupInfo;
                        ...
                        if (!dbGroup || dbGroup.memberVersion != ver.memberVersion) {
                            ...
                            const memberInfos: WL_IDbMemberInfo[] =
                                await dbInstance.convertFromMemberRaw(
                                    groupId,
                            ...
                            userIds = rspGroupInfo.memberInfoList.filter(value => {
                            ...
                            if (userIds.length) {
                                const buildGetUser = WeilaPBGroupWrapper.buildGetGroupMemberInfoReq(Long.fromValue(groupId), userIds);
                                if (buildGetUser.resultCode == 0) {
                                    const userRawInfos = await this.coreInterface.sendPbMsg(buildGetUser, 30000) as WL.Group.IRspGetMemberUserInfo;
    ...
    public async createGroup(name: string, groupType: number, avatar: string,
                             publicType: number, memberUserIdList: number[]): Promise<WL_IDbGroup|null> {
        if (avatar == '') {
        ...
        const buildResult = WeilaPBGroupWrapper.buildCreateGroupReq(name, groupType, avatar, 0, publicType, memberUserIdList);
        if (buildResult.resultCode == 0) {
            const rspCreateGroup = await this.coreInterface.sendPbMsg(buildResult) as WL.Group.IRspCreateGroup;
            ...
            const memberInfos = WeilaDB.getInstance().convertFromMemberRaw(groupId, rspCreateGroup.memberInfoList);
    ...
    public async inviteUser(groupId: string, userIdList: number[]): Promise<boolean> {
        const buildResult = WeilaPBGroupWrapper.buildGroupInviteReq(Long.fromValue(groupId), userIdList);
        if (buildResult.resultCode == 0) {
    ...
    public async getGroupOnlineMembers(groupId: string): Promise<number[]> {
        ...
        if (buildResult.resultCode == 0) {
            const rspOnlineMember = await this.coreInterface.sendPbMsg(buildResult) as WL.Group.IRspGetGroupOnlineMember;
    ...
    public async answerGroupInvitation(groupId: string, inviterId: number, status: number): Promise<boolean> {
        const buildMsgRet = WeilaPBGroupWrapper.buildGroupAnswerInviteReq(Long.fromValue(groupId), inviterId, status);
        if (buildMsgRet.resultCode == 0) {
    ...
    public async answerGroupJoin(groupId: string, joinId: number, status: number): Promise<boolean> {
        const buildMsgRet = WeilaPBGroupWrapper.buildGroupAnswerJoinReq(Long.fromValue(groupId), joinId, status);
        if (buildMsgRet.resultCode == 0) {
            await this.coreInterface.sendPbMsg(buildMsgRet);
            return true;
        }
    ...
    public async getGroupAttribute(groupId: string): Promise<WL_IDbGroup> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rsp = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Group.IRspGetGroupAttribute;
    ...
    public async deleteGroupMembers(groupId: string, userIds: number[]): Promise<boolean> {
        const buildMsgRet = WeilaPBGroupWrapper.buildDelGroupMemebrReq(Long.fromValue(groupId), userIds);
        if (buildMsgRet.resultCode == 0) {
    ...
    public async changeGroupOwner(groupId: string, ownerUserId: number): Promise<boolean> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rsp = await this.coreInterface.sendPbMsg(buildMsgRet) as WL.Group.IRspChangeGroupAttribute;
    ...
    public async joinGroup(groupId: string, detail?: string, password?: string, subUserId?: number): Promise<boolean> {
        if (subUserId) {
            const groupInfo = await WeilaDB.getInstance().getGroup(groupId);
        ...
        const buildMsgRet = WeilaPBGroupWrapper.buildGroupJoinReq(
            Long.fromValue(groupId), detail, password, subUserId);
        if (buildMsgRet.resultCode == 0) {
            await this.coreInterface.sendPbMsg(buildMsgRet);
        }
    }
    ...
    public async quitGroup(groupId: string, subUserId?: number): Promise<boolean> {
        ...
        const groupInfo = await WeilaDB.getInstance().getGroup(groupId);
        if (groupInfo.ownerId == this.coreInterface.getLoginUserInfo().userId &&
        ...
        let buildMsgRet: any;
        if (subUserId) {
            buildMsgRet = WeilaPBGroupWrapper.buildDelGroupMemebrReq(Long.fromValue(groupId), [subUserId], subUserId);
        }else {
            buildMsgRet = WeilaPBGroupWrapper.buildDelGroupMemebrReq(Long.fromValue(groupId), [this.coreInterface.getLoginUserInfo().userId]);
        }
    ...
    public async changeMemberType(groupId: string, memberUserId: number, memberType: number): Promise<boolean> {
    ...
    public async getDeviceSubGroups(subUserId: number): Promise<WL_IDbGroup[]> {
        ...
        if (buildMsgRet.resultCode == 0) {
            const rspGetSubGroupAttribute: WL.Group.IRspGetSubGroupAttribute = await this.coreInterface.sendPbMsg(buildMsgRet);
            let groupList:WL_IDbGroup[] = [];
            if (rspGetSubGroupAttribute.groupAttrList && rspGetSubGroupAttribute.groupAttrList.length) {
                groupList = await WeilaDB.getInstance().convertFromGroupRaw(rspGetSubGroupAttribute.groupAttrList);
            }
