...
class AliOssHelper {
    ...
    private async getKeysFromServer(): Promise<{ accessKeyId: string, accessKeySecret: string, stsToken: string }> {
        try {
            ...
            let response = await fetchWithTimeout('/v1/aliyun/sts?access-token=' + weilaToken, {method: 'GET'});
            const keyData: any = await response.json();
            console.log("keyData:", keyData);
            if (keyData.errcode == 0) {
                wllog('关键数据:', keyData);
                this.expireTimeout = keyData.data.Expiration;
                return {accessKeyId: keyData.data.AccessKeyId,
    ...
    private getUploadUrl(senderId: number, sessionId: string,
        ...
        let url = '/cache/' + dateStr + '/' + userStr + '/' + timeStr + '-user_' + senderId + '-seq_' + this.uploadSeq++;
    ...
    private processUploadData(): void {
        this.init().then(value => {
           if (!this.isUploading && this.uploadingQueue.length > 0) {
               ...
               client.put(uploadParams.remoteUrl, uploadParams.fileData, {timeout: 30000})
                   .then(objResult => {
    ...
    public async uploadToCache(senderId: number, sessionId: string,
                             sessionType: number, fileName: string, fileData: File): Promise<WL_UploadResult> {
    ...
    public async uploadToAvatar(userId: number, group: boolean, imageData: Blob, width: number, height: number): Promise<WL_UploadResult> {
        return new Promise<WL_UploadResult>((resolve, reject) => {
            // get date str
            let curDate = new Date();
