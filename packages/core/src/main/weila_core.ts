...
import {calculateOpusDataFrame, fetchWithTimeout, getMsgDataIdByCombo, getPbResultCodeMassage} from 'main/weila_utils';
...
class WeilaCore implements WL_CoreInterface {
    ...
    constructor() {
        ...
        this.mainFsm = mainFsm.withContext({
            loginParam: null,
            core: this
        }).withConfig({
            services: {
        ...
        // 主要状态机的激活
        this.mainFsmService = interpret(this.mainFsm).onTransition((state, event) => {
            wllog('事件:%s 从状态:%s ===> 状态:%s',
                JSON.stringify(event), state.history ? JSON.stringify(state.history.value): '初始状态', JSON.stringify(state.value));
        }).start();
    ...
    private processSendingPbMsg(): void {
        wllog('处理PB消息，队列有%d个等待发送消息', this.sendingPbMsgList.length, this.waitingTimeoutChecking)
    ...
    private async prepareData(context: any, event: any): Promise<boolean> {
        ...
        try {
            await this.businessModule.initService(sessionList.filter(value => {
    ...
    private async loginServer(context: any, event: any): Promise<WL_LoginResult> {
        ...
        try {
            wllog('开始登陆');
            loginResult.loginUserInfo = await this.loginModule.loginReq(loginParam.account, loginParam.password, loginParam.countryCode);
    ...
    private onSysTimeChecking(context: any, event: any, actionMeta: any) {
        wllog('--------->onSysTimeChecking', event, this.waitingRspMap.size, this.waitingTimeoutChecking);
     ...
     */
    public async weila_login(account: string, password: string, countryCode: string): Promise<WL_IDbUserInfo> {
        ...
        if (audioMsgData.msgType == WL_IDbMsgDataType.WL_DB_MSG_DATA_AUDIO_TYPE) {
            if (!audioMsgData.audioData.data && audioMsgData.audioData.audioUrl) {
                ...
                try {
                    ...
                    console.log("audioMsgData audioData is undefined ? ", audioMsgData.audioData == undefined);
                }
    ...
    public async weila_playHistoryList(audioMsgDatas: WL_IDbMsgData[]): Promise<boolean> {
        audioMsgDatas = audioMsgDatas.filter(value => {
            return (value.msgType == WL_IDbMsgDataType.WL_DB_MSG_DATA_AUDIO_TYPE &&
    ...
    public async weila_getMsgDatas(
        sessionId: string, sessionType: number,
        fromMsgId: number, count: number): Promise<WL_IDbMsgData[]> {
     ...
     */
    public async weila_changeMemberType(groupId: string, memberUserId: number, memberType: number): Promise<boolean> {
     ...
     * @param sessionType 会话类型
     */
    public async weila_getSessionFromDb(sessionId: string, sessionType: number): Promise<WL_IDbSession|undefined> {
     ...
     * @param sessionType 会话类型
     */
    public async weila_getSessionSetting(sessionId: string, sessionType: number): Promise<WL_IDbSessionSetting|undefined> {
    ...
    public async weila_updateSessionSetting(
        sessionId: string, sessionType: number,
        settingParam: WL_IDbSessionSettingParams): Promise<boolean> {
     ...
     */
    public async weila_startNewSession(sessionId: string, sessionType: number, extra?: any): Promise<WL_IDbSession> {
     ...
     */
    public async weila_sendPttAudioData(sessionId: string, sessionType: number, audioData: Uint8Array): Promise<boolean> {
     ...
     */
    public async weila_sendAudioMsg(sessionId: string, sessionType: number, audioUrl: string, frameCount?: number): Promise<boolean> {
        if (!this.isLoginReady) {
            return Promise.reject('微喇状态不正确');
        }
     ...
     */
    public async weila_sendPTTByAudioData(sessionId: string, sessionType: number, audioData: Uint8Array): Promise<boolean> {
        if (!this.isLoginReady) {
            return Promise.reject('微喇状态不正确');
        }
     ...
     * @param text 文字，字符串
     */
    public async weila_sendTextMsg(sessionId: string, sessionType: number, text: string): Promise<boolean> {
        if (!this.isLoginReady) {
            return Promise.reject('微喇状态不正确');
        }

        return this.sessionModule.sendMsgData(sessionId, sessionType, text, WL_IDbMsgDataType.WL_DB_MSG_DATA_TEXT_TYPE);
    }
    ...
    public async weila_sendPosition(sessionId: string, sessionType: number, position: WL_IDbLocationShared): Promise<boolean> {
        ...
        return this.sessionModule.sendMsgData(sessionId, sessionType, position, WL_IDbMsgDataType.WL_DB_MSG_DATA_LOCATION_TYPE);
    }

    private async weila_sendFileInfo(sessionId: string, sessionType: number,
                                     filename: string, data: File, msgType: WL_IDbMsgDataType): Promise<boolean> {
        if (!this.isLoginReady) {
            return Promise.reject('微喇状态不正确');
        }
        ...
        const uploadResult = await AliOssHelper.getInstance().uploadToCache(
            this.loginUserInfo!.userId, sessionId, sessionType, filename, data);
     ...
     */
    public async weila_sendImage(sessionId: string, sessionType: number, imageName: string, image: File): Promise<boolean> {
        return this.weila_sendFileInfo(sessionId, sessionType, imageName, image, WL_IDbMsgDataType.WL_DB_MSG_DATA_IMAGE_TYPE);
    }
     ...
     */
    public async weila_sendFile(sessionId: string, sessionType: number, filename: string, fileData: File): Promise<boolean> {
        return this.weila_sendFileInfo(sessionId, sessionType, filename, fileData, WL_IDbMsgDataType.WL_DB_MSG_DATA_FILE_TYPE);
    }
     ...
     */
    public async weila_sendVideo(sessionId: string, sessionType: number, videoName: string, video: File): Promise<boolean> {
        return this.weila_sendFileInfo(sessionId, sessionType, videoName, video, WL_IDbMsgDataType.WL_DB_MSG_DATA_VIDEO_TYPE);
    }
     ...
     * @param sessionType 会话类型
     */
    public async weila_getLocation(sessionId: string, sessionType: number): Promise<WL_IDbLocationInfo[]> {
     ...
     */
    public async weila_answerGroupJoin(groupId: string, joinId: number, answerStatus: number): Promise<boolean> {
     ...
     * @param remark 备注
     */
    public async weila_inviteFriend(inviteeId: number, detail: string, remark: string): Promise<boolean> {
     ...
     * @param detail 留言
     */
    public async weila_inviteFriendsForDevice(inviteeIds: number[], subUserId: number, detail: string): Promise<boolean> {
     ...
     * @param subUserId 设备id
     */
    public async weila_deleteFriendsForDevice(userIds: number[], subUserId: number): Promise<boolean> {
     ...
     * @param info 留言
     */
    public async weila_answerFriendInvite(inviterUserId: number, answerStatus: number, info: string): Promise<boolean> {
     ...
     */
    public async weila_answerGroupInvitation(groupId: string, invitorId: number, status: number): Promise<boolean> {
     ...
     */
    public async weila_createGroup(name: string, groupType: number, groupIcon: Blob|null,
                                   publicType: number, memberUserIdList: number[]): Promise<WL_IDbGroup> {
        let avatar = '';
        if (groupIcon) {
            try {
                const uploadResult = await AliOssHelper.getInstance().uploadToAvatar(this.loginUserInfo.userId, true, groupIcon, 100, 100);
                if (uploadResult.statusCode == 200) {
        ...
        const groupInfo: WL_IDbGroup = await this.groupModule.createGroup(name, groupType, avatar, publicType, memberUserIdList);
     ...
     */
    public async weila_joinGroup(groupId: string, detail?: string, password?: string): Promise<boolean> {
    ...
    public async weila_uploadImageCache(
        sessionId: string,
        sessionType: number,
        imageFile: File): Promise<string> {
        const name = new Date().toTimeString() + "_map.png";
        const result: WL_UploadResult =
            await AliOssHelper.getInstance().uploadToCache(
                this.loginUserInfo.userId, sessionId, sessionType, name, imageFile);
     ...
     * @param subUserId 设备id
     */
    public async weila_setDeviceConfig(cfgKey: string, cfgValue: string, subUserId: number): Promise<string> {
    ...
    public async weila_getServiceSession(sessionId: string): Promise<WL_IDbServiceSessionInfo|undefined> {
        const session: WL_IDbSession|undefined = await WeilaDB.getInstance().getSession(sessionId, WL_IDbSessionType.SESSION_SERVICE_TYPE);
        if (session) {
            ...
            if (service) {
                ...
                if (serviceSessionInfo.serviceSession) {
                    ...
                    serviceSessionInfo.staffs =
                        await WeilaDB.getInstance().getServiceStaffs(serviceSessionInfo.serviceSession.staffIds);
                    serviceSessionInfo.customer =
                        await WeilaDB.getInstance().getUser(serviceSessionInfo.serviceSession.customerId);
     ...
     * @param content 内容
     */
    public async weila_staffSessionInvite(sessionId: string, userIds: number[], content?: string): Promise<boolean> {
     ...
     * @param status 应答情况
     */
    public async weila_staffAnswerSessionInvite(sessionId: string, status: WL_AnswerStatus): Promise<boolean> {
     ...
     */
    public async weila_staffRemoveSessionStaff(sessionId: string, staffIds: number[]): Promise<boolean> {
     ...
     */
    public async weila_staffSearchStaffs(serviceId: number, pageIndex: number, pageSize: number, staffClass?: number): Promise<WL_IDbServiceStaffInfo[]> {
    ...
    public async weila_get_token(): Promise<string> {
        const tokenItem = await WeilaDB.getInstance().getSettingItem(WL_IDbSettingID.SETTING_LOGIN_TOKEN);
