...
const mainFsm = createMachine({
  ...
  states: {
    uninit: {
      ...
      on: {
        FSM_LOAD_RESOURCE_EVT: {
          description:
            '此事件由init调用触发，传入promise作为事件的数据，后续成功与否可以通过此通知调用者\n',
    ...
    loadingResource: {
      ...
      description:
        '这是加载资源的状态，加载wasm等资源。只有加载成功，后续才可以继续，否则失败后跳到异常状态',
    },
    exception: {
      entry: 'onException',
      type: 'final',
    ...
    loginTry: {
      description:
        '登录尝试状态，会自动启动每2秒重新返回登录流程。如果尝试次数超了或失败因为特殊原因，则不应该尝试了\n',
    ...
    ready: {
      entry: ['onReadyEntry', send('FSM_SYSTEM_WATCH_TIMEOUT_EVT')],
      exit: 'onReadyExit',
      ...
      states: {
        ...
        checkPttSource: {
          invoke: {
            ...
            onDone: {
              actions: send((_, event: any) => ({
                type: 'FSM_PLAY_NEXT_PTT_ITEM_EVT',
                data: event.data,
              })),
