...
class WeilaDB extends Dexie {
    ...
    constructor(userId: string, version: number) {
        ...
        this.version(version).stores({
            ...
            msgDatas: 'combo_id, sessionId, sessionType, [sessionId+sessionType], msgType, created, msgId',
            settings: 'id',
    ...
    @AsyncTime()
    public async convertFromSessionRaw(sessionRaws: WL.Session.ISessionInfo[]): Promise<WL_IDbSession[]> {
        ...
        for (let i = 0; i < sessionCount; i++) {
            ...
            if (sessionRaws[i].latestMsgData) {
                const msgDatas: WL_IDbMsgData[] = this.convertFromMsgDataRaws([sessionRaws[i].latestMsgData],
                    this.loginUserId);
    ...
    @AsyncTime()
    public async getSessionSetting(sessionId: string, sessionType: number): Promise<WL_IDbSessionSetting | undefined> {
    ...
    @AsyncTime()
    public async updateSessionSetting(sessionId: string, sessionType: number, params: WL_IDbSessionSettingParams): Promise<boolean> {
    ...
    @AsyncTime()
    public async getSession(sessionId: string, sessionType: number): Promise<WL_IDbSession | undefined> {
        const id = sessionId + "_" + sessionType;
    ...
    @AsyncTime()
    public async getGroupsSortByProperty(property: 'pinyinName' | 'groupType' | 'created'): Promise<WL_IDbGroup[]> {
    ...
    @SyncTime()
    public convertFromMemberRaw(groupId: string, memberRaws: WL.Group.IGroupMemberInfo[]): WL_IDbMemberInfo[] {
    ...
    public async delMemberInfos(groupId: string, userIds: number[]): Promise<void> {
        const count = this.group_members.where('userId').anyOf(userIds).and(x => {
    ...
    public async getGroupMembers(groupId: string): Promise<WL_IDbGroupMember[]> {
        const memberInfos: WL_IDbMemberInfo[] = await this.group_members.where('groupId').equals(groupId).toArray();
    ...
    @AsyncTime()
    public async getGroupMemberInfo(groupId: string, userId: number): Promise<WL_IDbMemberInfo | undefined> {
        const id = groupId + "_" + userId;
        return this.group_members.get(id);
    }

    @AsyncTime()
    public async getGroupMember(groupId: string, userId: number): Promise<WL_IDbGroupMember | undefined> {
        const groupMember = {} as WL_IDbGroupMember;
        const id = groupId + "_" + userId;
    ...
    public async getGroupAdminMembers(groupId: string): Promise<WL_IDbGroupMember[]> {
        ...
        const memberInfos: WL_IDbMemberInfo[] =
            ...
            .equals([groupId, WL_IDbMemberType.ADMIN_MEMBER_TYPE]).toArray();
    ...
    @AsyncTime()
    public async getMsgDataByMsgIds(sessionId: string, sessionType: number, msgIds: number[]): Promise<WL_IDbMsgData[]> {
        return this.msgDatas.where('msgId').anyOf(msgIds).and(x => {
            return x.sessionId == sessionId && x.sessionType == sessionType;
        }).sortBy('created');
    }

    @SyncTime()
    public convertFromMsgDataRaws(msgDataRaws: WL.Session.IMsgData[], myUserId: number): WL_IDbMsgData[] {
            ...
            switch (msgDataRaws[i].msgType) {
                ...
                case WL.Session.MessageType.MSG_TYPE_AUDIO: {
                    ...
                    if (msgDataRaws[i].audioData) {
                        console.log('audioData is ', msgDataRaws[i].audioData.payload, msgDataRaws[i].audioData.url);
                    }
    ...
    @AsyncTime()
    public async getLastMsgDatas(sessionId: string, sessionType: number, count: number): Promise<WL_IDbMsgData[]> {
        wllog('获取最后%d条消息:', count);
        return this.msgDatas.orderBy('created').and(x => {
            return x.sessionId == sessionId && x.sessionType == sessionType;
        }).reverse().limit(count).toArray();
    }

    @AsyncTime()
    public async getMsgDatasWithRange(sessionId: string, sessionType: number, fromMsgId: number, count: number): Promise<WL_IDbMsgData[]> {
        if (fromMsgId < count) {
            count = fromMsgId;
        }
    ...
    @AsyncTime()
    public async getUnReadAudioMsg(sessionId: string, sessionType: number, sinceMsgId: number): Promise<WL_IDbMsgData[]> {
        return this.msgDatas.where('[sessionId+sessionType]').equals([sessionId, sessionType]).and(x => {
            return x.msgId >= sinceMsgId && x.msgType == WL_IDbMsgDataType.WL_DB_MSG_DATA_AUDIO_TYPE && x.status == WL_IDbMsgDataStatus.WL_DB_MSG_DATA_STATUS_NEW;
        }).sortBy('created');
    }

    @AsyncTime()
    public async getMsgDataByMsgId(sessionId: string, sessionType: number, msgId: number): Promise<WL_IDbMsgData | undefined> {
        return this.msgDatas.where('[sessionId+sessionType]').equals([sessionId, sessionType]).and(x => {
    ...
    public async delMsgDatas(sessionId: string, sessionType: number): Promise<void> {
        const deletedNum = await this.msgDatas.where('[sessionId+sessionType]').equals([sessionId, sessionType]).delete();
        wllog('批量删除消息%d个', deletedNum);
    }

    @AsyncTime()
    public async getLastMsgData(sessionId: string, sessionType: number): Promise<WL_IDbMsgData | undefined> {
        return this.msgDatas.orderBy('created').and(x => {
            return x.sessionId == sessionId && x.sessionType == sessionType;
        }).last();
    }
    ...
    @AsyncTime()
    public async getNotifications(notificationTypes: WL_IDbNotificationType[]): Promise<WL_IDbNotification[]> {
        const notifications =
            await this.notifications.where('notificationType').anyOf(notificationTypes).sortBy('createTime');
    ...
    @SyncTime()
    public convertFromExtensionInfoRaw(extensionInfoRaw: WL.Login.IExtensionInfo): WL_IDbExtensionInfo {
    ...
    @SyncTime()
    public convertFromServiceSessionRaw(serviceSession: WL.Business.IServiceSession): WL_IDbServiceSessionInfo {
        ...
        const userInfos = this.convertFromUserRaw(serviceSession.staffAttributes.map(value => {
        ...
        serviceSessionInfo.customer = this.convertFromUserRaw([serviceSession.customerAttribute.userInfo])[0];
        ...
        serviceSessionInfo.serviceSession.createdTime = Long.fromValue(serviceSession.created).toNumber();
        serviceSessionInfo.serviceSession.updatedTime = Long.fromValue(serviceSession.updated).toNumber();
