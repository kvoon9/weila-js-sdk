...
import {
  ...
  WLLocationInfo,
  WLSessionInfo,
} from './weilapb_wrapper_data';
import Long from 'long';
import { BuildWeilaMsg } from './weilapb_wrapper';

class WeilaPbLocationWrapper {
  public static buildReportLocationReq(
    locationInfo: WLLocationInfo,
    ...
    locationMessage.reqReportLocation.locationInfo.locationParam.timestamp = Long.fromValue(
      locationInfo.timestamp,
    );

    locationMessage.reqReportLocation.sessionInfos = [];
    sessionInfoList.forEach((value) => {
      locationMessage.reqReportLocation!.sessionInfos!.push({
        sessionId: Long.fromValue(value.sessionId),
        sessionType: value.sessionType,
      });
    });

    return BuildWeilaMsg.buildWeilaMsgReq(
      WL.Service.ServiceID.SERVICE_LOCATION,
      WL.Location.LocationCommandId.LOCATION_COMMAND_REPORT_LOCATION,
      WL.Service.CommandType.COMMAND_REQUEST,
      locationMessage,
    );
  }

  public static buildGetLocationReq(
    sessionInfo: WLSessionInfo,
    userIdList: number[],
  ): WLBuildMsgRet {
    const locationMessage = new WL.Location.LocationMessage();
    ...
    return BuildWeilaMsg.buildWeilaMsgReq(
      WL.Service.ServiceID.SERVICE_LOCATION,
      WL.Location.LocationCommandId.LOCATION_COMMAND_GET_LOCATION,
      WL.Service.CommandType.COMMAND_REQUEST,
      locationMessage,
    );
  }

  public static buildLocationControlReq(
    sessionInfo: WLSessionInfo,
    locationControl: WLLocationControl,
    userIdList: number[],
  ): WLBuildMsgRet {
    const locationMessage = new WL.Location.LocationMessage();
    ...
    locationMessage.reqLocationControl.sessionInfo.sessionId = Long.fromValue(
    ...
    return BuildWeilaMsg.buildWeilaMsgReq(
      WL.Service.ServiceID.SERVICE_LOCATION,
      WL.Location.LocationCommandId.LOCATION_COMMAND_LOCATION_CONTROL,
      WL.Service.CommandType.COMMAND_REQUEST,
      locationMessage,
    );
  }
